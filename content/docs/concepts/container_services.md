---
summary: 了解容器服务以及它们如何帮助保持代码库干净且可测试。
---

# Container services

正如我们在 [IoC 容器指南](./dependency_injection.md#container-bindings), 容器绑定使您的代码库免于编写样板代码来构造对象才能使用。

容器绑定使您的代码库免于编写样板代码来构造对象才能使用。

在下面的示例中，在使用 `Database` 类之前，您必须创建它的一个实例。根据您正在构造的类，您可能需要编写大量样板代码来获取其所有依赖项。
...
然而，当使用 IoC 容器时，您可以将构造类的任务委托给容器，并获取预构建的实例。
...
使用容器解析预配置对象很好。但是，使用 `container.make` 方法也有其缺点。

- 编辑器对自动导入功能很擅长。如果您尝试使用一个变量，编辑器能够猜测该变量的导入路径，那么它会为您编写导入语句。 **但是，这不能与 `container.make` 调用一起工作**

- 使用导入语句和 `container.make` 调用的混合方式相比统一的导入/使用模块语法来说感觉不太直观。

为了克服这些缺点，我们将 `container.make` 调用包装在一个普通的 JavaScript 模块中，这样您就可以使用 `import` 语句获取它们.

例如, `@adonisjs/lucid` 包有一个名为 `services/db.ts` 的文件，这个文件大致包含以下内容。
...
如您所见，我们仍然依赖容器为我们解析 Database 类的实例。但是，通过一层间接调用，我们可以将 `container.make` 调用替换为常规的 `import` 语句。

**包装 `container.make` 调用的 JavaScript 模块被称为容器服务** 几乎每个与容器交互的包都附带一个或多个容器服务。

## 容器服务与依赖注入

容器服务是依赖注入的替代方案。例如，不接受 `Disk` 类作为依赖项，而是请求 `drive` 服务为您提供一个磁盘实例。让我们看一些代码示例.

在以下示例中，我们使用 `@inject` 装饰器注入 `Disk` 类的实例。
...
当使用 `drive` 服务时，我们调用 `drive.use`方法获取使用 `s3` 驱动程序的 `Disk` 实例。
...
容器服务非常适合让您的代码简洁。而依赖注入则在不同应用部分之间创建松耦合。

选择其中一个取决于您的个人喜好和您希望采用的代码结构方法。

## 使用容器服务进行测试

依赖注入的直接好处是能够在编写测试时交换依赖项。

为了在容器服务方面提供类似的测试体验，AdonisJS 提供了一流的 API 来在编写测试时伪造实现。

在以下示例中，我们调用 `drive.fake` 方法将驱动磁盘替换为内存中的驱动程序。创建伪造后，任何对 `drive.use` 方法的调用都将收到伪造的实现。
